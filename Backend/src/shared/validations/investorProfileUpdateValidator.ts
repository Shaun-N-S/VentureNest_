  ...
  .object({
    ...
    formData: z.object({
      ...
      investmentMin: z.preprocess(
        toNumber,
        z.number().positive("Minimum investment must be greater than 0").optional()
      ),

      investmentMax: z.preprocess(
        toNumber,
        z.number().positive("Maximum investment must be greater than 0").optional()
      ),
    }),
  })
  .refine(
    (data) =>
      data.formData.investmentMin === undefined ||
      data.formData.investmentMax === undefined ||
      data.formData.investmentMax >= data.formData.investmentMin,
    {
      message: "Maximum investment must be greater than or equal to minimum investment",
      path: ["formData", "investmentMax"],
    }
  );
