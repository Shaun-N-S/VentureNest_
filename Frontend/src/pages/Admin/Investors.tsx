...
const Investors: React.FC = () => {
    ...
    const investors: Investor[] = useMemo(() => data?.data?.investors || [], [data]);
    ...
    const handleStatusToggle = useCallback(
        async (investorId: string, currentStatus: "ACTIVE" | "BLOCKED") => {
                ...
                { investorId, currentStatus },
                {
                    ...
                    onError: (err) => {
                        if (err instanceof AxiosError) {
                            const errorMsg = err.response?.data?.message || "Failed to update investor status";
                                ...
                                investors.map((investor, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <button
                                                onClick={() => handleStatusToggle(investor._id, investor.status)}
                                                disabled={isUpdating}
                                                className={`px-4 py-1.5 rounded-full text-xs font-semibold transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${investor.status === "ACTIVE"
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center">
                                        <div className="text-gray-400">
                                            ...
                                            <p className="italic">No investors found</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
                    ...
                    <div className="flex justify-center items-center mt-6 w-full">
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className={`px-4 py-2 rounded-md font-medium transition duration-200 ${page === 1 || isUpdating
                            ...
                            <button
                                ...
                                className={`px-4 py-2 rounded-md font-medium transition duration-200 ${page === totalPages || isUpdating
                                        ? "bg-gray-200 text-gray-400 cursor-not-allowed"
                                        : "bg-indigo-100 text-indigo-700 hover:bg-indigo-200"
                                    }`}
                            >
