...
import { useFetchAllProjects, useLikeProject } from "../../hooks/Project/projectHooks"
...
const ProjectPage = () => {
    ...
    // Debounced values for API calls (prevents constant filtering)
    const debouncedSearch = useDebounce(search, 500);
    const debouncedStage = useDebounce(stage, 300);
    const debouncedSector = useDebounce(sector, 300);

    // Only API call happens with debounced values
        ...
        likeProject(projectId, {
            onSuccess: (res) => {
                ...
                queryClient.setQueryData<PersonalProjectApiResponse>(
                    ["projects", 1, 10, debouncedSearch, debouncedStage, debouncedSector],
                    (old) => {
